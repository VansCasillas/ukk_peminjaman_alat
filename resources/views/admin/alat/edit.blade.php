...
<div class="container mt-4">
    <h2 class="mb-4">Edit Alat</h2>

    <form action="{{ route('admin.alat.update', $alat->id) }}" method="POST" class="bg-white p-4 rounded shadow-sm">
        @csrf
        @method('PUT')

        <div class="mb-3">
            <label for="name" class="form-label fw-semibold text-dark">Nama Alat</label>
            <input type="text" id="name" name="nama_alat" class="form-control styled-input" value="{{ old('nama_alat', $alat->nama_alat) }}">
        ...
        <div class="mb-3">
            ...
            <select name="kategori_id" id="jenis_kategori" class="form-control styled-input">
                ...
                @foreach ($kategori as $ktr)
                    <option value="{{ $ktr->id }}" {{ $alat->kategori_id == $ktr->id ? 'selected' : '' }}>{{ $ktr->nama_kategori }}</option>
                @endforeach
            </select>
        ...
        <div class="mb-3">
            ...
            <input type="text" id="qty" name="qty" class="form-control styled-input" value="{{ old('qty', $alat->qty) }}">
